...
function renderChatOptions() {
  ...
  $("chatOptionsContent").innerHTML = `
    <section class="options-section"><div class="section-title"><strong>传讯模式</strong></div><div class="mode-choice"><button data-mode="random" class="${state.mode === "random" ? "active" : ""}">随机<span>当前开启的字卡与表情包等量进入抽取池</span></button><button data-mode="response" class="${state.mode === "response" ? "active" : ""}">回应<span>命中关键词时，从命中分区合并池保证一条</span></button></div></section>
...
function buildReplyItems(combined, conversationMessages = currentMessages(), mode = state.mode) {
  ...
  const pool = [...cardPool.map((card) => ({ kind: "card", card })), ...(state.replyStickersDisabled ? [] : state.stickers.map((sticker) => ({ kind: "sticker", sticker })))];
...
function renderCardsDrawer() {
  ...
  const groupedCards = visibleSections.map((name) => {
    ...
    return `<details class="card-group ${rules.enabled ? "" : "is-disabled"}" ${query ? "open" : ""}><summary><span><strong>${escapeHtml(name)}<i class="section-badges">${rules.enabled ? "" : '<em class="disabled-badge">未启用</em>'}${rules.combo ? '<em class="combo-badge">可组合</em>' : ""}</i></strong><small data-section-count="${escapeHtml(name)}">${cards.length} 张字卡</small></span><b>⌄</b></summary><div class="card-group-editor"><div class="section-rule-toggles">${ruleToggle("random", "随机")}${ruleToggle("response", "回应")}${ruleToggle("enabled", "启用")}${ruleToggle("combo", "组合")}</div><label class="section-trigger-setting"><span>回应关键词</span><input data-section-triggers="${escapeHtml(name)}" value="${escapeHtml((rules.triggers || []).join("，"))}" placeholder="例如：想你，晚安，难过" spellcheck="false" autocomplete="off"></label><p>回应模式始终保留所有已开启分区；同时命中多个分区时，会合并成一个候选池并从中保证抽取一条。一行一张字卡，可直接在下方增删修改。</p><textarea class="section-card-editor" data-card-section="${escapeHtml(name)}" placeholder="在这里输入字卡，一行一张" spellcheck="false" autocapitalize="off" autocomplete="off">${escapeHtml(contents)}</textarea><button class="group-save-button" data-save-section="${escapeHtml(name)}">保存这个分区</button></div></details>`;
  }).join("");
  $("drawerContent").innerHTML = `
    <section class="drawer-section"><div class="section-title"><strong>分区</strong><span>${state.sections.length} 个</span></div><div class="card-library-stats"><span><b data-total-card-count>${state.cards.length}</b><small>全部字卡</small></span><span><b data-enabled-card-count>${enabledCardCount}</b><small>当前启用</small></span></div><div class="inline-form"><input id="newSection" placeholder="新分区名称"><button class="secondary-button" id="addSection">添加</button></div><div class="section-tags">${state.sections.map((name) => `<span class="section-chip">${escapeHtml(name)}${state.sections.length > 1 ? `<button data-delete-section="${escapeHtml(name)}">×</button>` : ""}</span>`).join("")}</div></section>
  ...
  document.querySelectorAll("[data-section-rule]").forEach((input) => input.addEventListener("change", () => {
    ...
    if (rule === "combo" || rule === "enabled") {
      ...
      if (badges) badges.innerHTML = `${rules.enabled ? "" : '<em class="disabled-badge">未启用</em>'}${rules.combo ? '<em class="combo-badge">可组合</em>' : ""}`;
