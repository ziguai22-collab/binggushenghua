...
function normalizeState(saved = {}) {
    ...
    migrated.replyStickerProbability = Math.min(100, Math.max(0, Number(saved.replyStickerProbability ?? defaults.replyStickerProbability)));
...
function renderChatOptions() {
  ...
  $("proactiveEnabled").closest(".options-section").insertAdjacentHTML("beforebegin", `<section class="options-section"><div class="section-title"><strong>每轮回复条数</strong><span>随机生成</span></div><div class="range-field"><div class="range-head"><span>最多回复</span><b id="replyMaxValue">${state.replyMaxCount} 条</b></div><input id="replyMaxRange" type="range" min="1" max="6" step="1" value="${state.replyMaxCount}"></div><p class="options-note">每轮会在 1 条到所选上限之间随机，不会固定发满。</p></section><section class="options-section"><div class="section-title"><strong>回复表情包</strong><span>整库随机</span></div><div class="range-field"><div class="range-head"><span>发表情包概率</span><b id="replyStickerValue">${state.replyStickerProbability}%</b></div><input id="replyStickerRange" type="range" min="0" max="100" step="5" value="${state.replyStickerProbability}"></div><p class="options-note">每条回复按此概率从全部表情包中随机抽取；没有可用表情时自动改发字卡。</p></section>`);
  ...
  $("replyStickerRange").addEventListener("input", (event) => { state.replyStickerProbability = Number(event.target.value); $("replyStickerValue").textContent = `${state.replyStickerProbability}%`; saveState(); });
...
function buildReplyItems(combined, conversationMessages = currentMessages(), mode = state.mode) {
  ...
  const availableCards = cardPool.map((card) => ({ kind: "card", card }));
  const availableStickers = state.stickers.map((sticker) => ({ kind: "sticker", sticker }));
  if (!availableCards.length && !availableStickers.length) return [{ kind: "text", content: "这次还没有合适的话。先去字卡里添上一句吧。" }];
  const maximum = Math.min(Math.max(1, Number(state.replyMaxCount) || 1), availableCards.length + availableStickers.length);
  const count = 1 + Math.floor(Math.random() * maximum);
  const replies = [];
  while (replies.length < count && (availableCards.length || availableStickers.length)) {
    const stickerChance = Math.min(100, Math.max(0, Number(state.replyStickerProbability) || 0));
    const useSticker = availableStickers.length && (!availableCards.length || Math.random() * 100 < stickerChance);
    const source = useSticker ? availableStickers : availableCards;
    const picked = source.splice(Math.floor(Math.random() * source.length), 1)[0];
    ...
    if (picked.card.combo && Math.random() < 0.5) {
      ...
      if (followPool.length) {
        ...
        const followIndex = availableCards.findIndex((item) => item.card.id === follow.id);
        if (followIndex >= 0) availableCards.splice(followIndex, 1);
