...
function renderConversationsDrawer() {
  ...
  const historyRow = (conversation) => { const last = conversation.messages.at(-1); return `<article class="history-row" data-history-row="${conversation.id}"><button class="history-main" data-conversation="${conversation.id}"><strong>${escapeHtml(conversation.title)}</strong><small>${escapeHtml(last?.content || "还没有消息")}</small></button><time>${dateText(conversation.updatedAt)}</time><button class="history-edit" data-edit-conversation="${conversation.id}" aria-label="编辑对话名称">${pencilIcon}</button><button class="history-delete" data-delete-conversation="${conversation.id}" aria-label="删除历史对话">${deleteIcon}</button><input class="history-name-input" data-conversation-name="${conversation.id}" value="${escapeHtml(conversation.title)}" hidden></article>`; };
