...
function shownSettingColor(key) {
  return key === "choiceCardColor" ? (safeColor(state.choiceCardColor) || defaultChoiceCardColor()) : shownBubbleColor(key);
}
...
function renderPlayMessage(message) {
  ...
  if (message.type === "dice") {
    ...
    return `<div class="bubble play-throw dice-card"><div class="die-scene"><div class="die-cube value-${value} ${animate ? "rolling" : ""}" style="--die-final:var(--die-${value})">${diceFace(value)}</div></div><strong>掷出 ${value}</strong></div>`;
  }
  if (message.type === "rps") {
