...
function buildReplyItems(combined, conversationMessages = currentMessages(), mode = state.mode) {
  ...
  if (mode === "response") {
    ...
    enabledSections.forEach((section) => {
      ...
      if (!triggers.length) fallbackSections.add(section);
      else if (combined && triggers.some((word) => combined.includes(word))) matchedSections.add(section);
    });
    const allowedSections = matchedSections.size ? matchedSections : fallbackSections;
    cardPool = eligibleCards.filter((card) => allowedSections.has(card.section));
  }
  ...
  if (mode === "response" && matchedSections.size && cardPool.length) {
    const requiredCard = cardPool[Math.floor(Math.random() * cardPool.length)];
    replies.push({ kind: "card", card: requiredCard });
  }
...
function bindGlobalEvents() {
  ...
  document.addEventListener("keydown", (event) => { if (event.key === "Escape" && !$("mediaViewer").hidden) closeMediaViewer(); });
