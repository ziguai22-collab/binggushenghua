...
function renderMessages(scrollToEnd = true) {
  ...
  $("messages").innerHTML = currentMessages().map((message, index) => {
    ...
    return `${divider}<article class="message-row ${mine ? "me" : "lover"}" data-message-row="${message.id}">${mine ? "" : `<div class="message-avatar avatar lover-mark">${avatarMarkup(state.loverAvatar, state.loverName)}</div>`}<div class="message-body">${content}</div>${actions}${mine ? `<div class="message-avatar avatar me-mark">${avatarMarkup(state.myAvatar, state.myName)}</div>` : ""}</article>`;
  }).join("");
