...
function currentPointBalance() {
  return Math.max(0, normalizePointAmount(state.pointLedger.reduce((total, item) => total + Number(item.amount || 0), 0)));
}

function normalizePointAmount(value) {
  const number = Number(value);
  return Number.isFinite(number) ? Math.round(number * 100) / 100 : 0;
}

function parsePointCard(content) {
  const matched = String(content || "").trim().match(/^积分\s*\+\s*((?:\d+(?:\.\d{1,2})?)|(?:\.\d{1,2}))$/);
  if (!matched) return 0;
  const value = normalizePointAmount(matched[1]);
  return value > 0 ? Math.min(9999, value) : 0;
}

function addPointLedgerEntry({ amount, type, reason = "", note = "", sourceMessageId = "", product = null }) {
  const value = normalizePointAmount(amount);
  if (!value || (sourceMessageId && state.pointLedger.some((item) => item.sourceMessageId === sourceMessageId))) return false;
  state.pointLedger.push({ id: uid(), amount: value, type, reason, note, sourceMessageId, product, createdAt: new Date().toISOString() });
  return true;
}
...
function normalizeState(saved = {}) {
    ...
    migrated.fixedSections = [...new Set([...(Array.isArray(saved.fixedSections) ? saved.fixedSections : []), ...defaults.fixedSections].map(String).filter(Boolean))];
    const savedSections = Array.isArray(saved.sections) ? saved.sections.map((item) => typeof item === "string" ? item : item.name).filter(Boolean) : structuredClone(defaults.sections);
    migrated.sections = [...migrated.fixedSections, ...savedSections.filter((section) => !migrated.fixedSections.includes(section))];
    ...
    migrated.shopProducts = Array.isArray(saved.shopProducts) ? saved.shopProducts.map((product, index) => ({
      id: product.id || uid(), name: String(product.name || "未命名商品").slice(0, 80), description: String(product.description || "").slice(0, 300),
      price: Math.max(1, Math.trunc(Number(product.price) || 1)), cover: safeImage(product.cover), enabled: product.enabled === true,
      allowRepeat: product.allowRepeat !== false, sort: Number.isFinite(Number(product.sort)) ? Number(product.sort) : index,
    })) : [];
    migrated.pointLedger = Array.isArray(saved.pointLedger) ? saved.pointLedger.map((item) => ({
      id: item.id || uid(), amount: normalizePointAmount(item.amount), type: String(item.type || "adjustment"), reason: String(item.reason || ""), note: String(item.note || ""),
      sourceMessageId: String(item.sourceMessageId || ""), product: item.product && typeof item.product === "object" ? item.product : null, createdAt: item.createdAt || new Date().toISOString(),
    })).filter((item) => item.amount) : [];
    migrated.purchasedItems = Array.isArray(saved.purchasedItems) ? saved.purchasedItems.map((item) => ({
      id: item.id || uid(), productId: String(item.productId || ""), name: String(item.name || "已购商品"), description: String(item.description || ""),
      price: Math.max(1, Math.trunc(Number(item.price) || 1)), cover: safeImage(item.cover), boughtAt: item.boughtAt || new Date().toISOString(), usedAt: item.usedAt || "",
    })) : [];
    ...
    migrated.conversations = Array.isArray(saved.conversations) && saved.conversations.length ? saved.conversations.map((conversation) => {
      const messages = Array.isArray(conversation.messages) ? conversation.messages.map((message) => ({ id: message.id || uid(), type: "text", ...message })) : [];
      let replyHandledUserMessageId = typeof conversation.replyHandledUserMessageId === "string" ? conversation.replyHandledUserMessageId : "";
      if (!replyHandledUserMessageId) {
        const lastLoverIndex = messages.reduce((last, message, index) => message.from === "lover" ? index : last, -1);
        replyHandledUserMessageId = messages.slice(0, lastLoverIndex + 1).reverse().find((message) => message.from === "me")?.id || "";
      }
      return {
        ...
        messages, replyHandledUserMessageId,
      };
    }) : [{ id: uid(), title: `与${migrated.loverName}的对话`, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), messages: legacyMessages.map((message) => ({ id: message.id || uid(), type: "text", ...message })), replyHandledUserMessageId: "" }];
    ...
    migrated.autoReplyEnabled = saved.autoReplyEnabled === true;
    migrated.replyStickersDisabled = saved.replyStickersDisabled === true;
...
async function persistState(quiet = true) {
  ...
  const snapshot = structuredClone({ ...state, version: 17, lastSavedAt: savedAt });
  const task = async () => {
    try {
...
function renderChatOptions() {
  const conversation = currentConversation();
  $("chatOptionsContent").innerHTML = `
    <section class="options-section"><div class="section-title"><strong>传讯模式</strong></div><div class="mode-choice"><button data-mode="random" class="${state.mode === "random" ? "active" : ""}">随机<span>全部开启内容直接进入同一个抽取池</span></button><button data-mode="response" class="${state.mode === "response" ? "active" : ""}">回应<span>命中关键词时，从命中分区合并池保证一条</span></button></div></section>
    <section class="options-section reply-behavior-settings"><div class="section-title"><strong>回复行为</strong><span>触发与内容</span></div><label class="option-toggle"><span><b>自动回复</b><small>发送后自动触发；关闭时仍可点右下角手动回复</small></span><input id="autoReplyEnabled" type="checkbox" ${state.autoReplyEnabled ? "checked" : ""}><i></i></label><label class="option-toggle"><span><b>禁用表情包回复</b><small>只限制爱人的普通与主动回复，不影响你发送表情包</small></span><input id="replyStickersDisabled" type="checkbox" ${state.replyStickersDisabled ? "checked" : ""}><i></i></label></section>
    <section class="options-section"><div class="section-title"><strong>引用消息</strong><span>本轮消息</span></div><label class="option-toggle"><span><b>允许随机引用</b><small>文字和表情包回复都可能附带引用</small></span><input id="replyQuoteEnabled" type="checkbox" ${state.replyQuoteEnabled ? "checked" : ""}><i></i></label><div class="range-field"><div class="range-head"><span>引用概率</span><b id="replyQuoteValue">${state.replyQuoteProbability}%</b></div><input id="replyQuoteRange" type="range" min="0" max="100" step="5" value="${state.replyQuoteProbability}" ${state.replyQuoteEnabled ? "" : "disabled"}></div></section>
    <section class="options-section"><div class="section-title"><strong>主动传讯</strong><span>可选</span></div><label class="option-toggle"><span><b>允许主动发消息</b><small>页面打开或再次回来时检查</small></span><input id="proactiveEnabled" type="checkbox" ${state.proactiveEnabled ? "checked" : ""}><i></i></label><div class="range-field"><div class="range-head"><span>大约间隔</span><b id="proactiveValue">${state.proactiveInterval} 分钟</b></div><input id="proactiveRange" type="range" min="5" max="120" step="5" value="${state.proactiveInterval}"></div><p class="options-note">受手机系统限制，网页被彻底关闭后无法保证实时后台运行；重新打开时会补做检查。</p></section>
    <section class="options-section conversation-actions"><div class="section-title"><strong>当前对话</strong><span>${escapeHtml(conversation.title)}</span></div><button class="danger-outline-button" id="clearConversation">清除此对话的聊天记录</button></section>`;
  ...
  $("autoReplyEnabled").addEventListener("change", (event) => {
    state.autoReplyEnabled = event.target.checked;
    saveState();
    if (state.autoReplyEnabled && pendingMessages().length) queueAutoReply();
    else if (!state.autoReplyEnabled) { clearTimeout(autoReplyTimer); autoReplyTimer = null; autoReplyQueued = false; autoReplyConversationId = ""; }
  });
  $("replyStickersDisabled").addEventListener("change", (event) => { state.replyStickersDisabled = event.target.checked; saveState(); });
  ...
  $("clearConversation").addEventListener("click", () => {
    ...
    conversation.messages = []; conversation.replyHandledUserMessageId = ""; touchConversation(); saveState(); renderMessages(); renderChatOptions();
  });
...
function deliverProactiveMessage() {
  ...
  const stickers = state.replyStickersDisabled ? [] : state.stickers.map((sticker) => ({ kind: "sticker", sticker }));
  ...
  if (item?.kind === "sticker") currentMessages().push({ id: uid(), from: "lover", type: "sticker", content: `[表情] ${item.sticker.name}`, dataUrl: item.sticker.dataUrl, createdAt: new Date().toISOString() });
    ...
    const message = { id: uid(), from: "lover", type: "text", content: item.card.content, createdAt: new Date().toISOString() };
    const pointReward = parsePointCard(item.card.content);
    if (pointReward) { message.payload = { pointReward }; if (addPointLedgerEntry({ amount: pointReward, type: "card_reward", reason: "积分字卡", note: item.card.content, sourceMessageId: message.id })) showToast(`他收到了 ${pointReward} 枚积分`); }
...
function pendingShopRequests(conversation) {
  return conversation.messages.filter((message) => message.from === "me" && message.type === "shop-request" && message.status !== "resolved");
}

function sendShopRequest() {
  closePopovers();
  const conversation = currentConversation();
  if (pendingShopRequests(conversation).length) { showToast("上一份商城邀请还在等他挑选"); return; }
  if (!state.shopProducts.some((product) => product.enabled)) { showToast("先去商城上架至少一件商品"); return; }
  const message = { id: uid(), from: "me", type: "shop-request", content: "[商城邀请] 去挑一件喜欢的东西", status: "pending", payload: {}, quote: takeManualReplyQuote(), createdAt: new Date().toISOString() };
  conversation.messages.push(message);
  if (conversation.title === "新对话") conversation.title = "一起逛商城";
  touchConversation(); saveState(); renderMessages(); queueAutoReply();
}

function eligibleShopProducts() {
  const balance = currentPointBalance();
  return state.shopProducts.filter((product) => product.enabled && product.price <= balance && (product.allowRepeat || !state.purchasedItems.some((item) => item.productId === product.id && !item.usedAt)));
}
...
function renderPlayMessage(message) {
  ...
  if (message.type === "tarot-result") {
    ...
    return `<div class="bubble play-card tarot-result-card"><div class="tarot-result-preview">${cards.map((card, index) => `<div class="tarot-chat-card" style="--tarot-stack-index:${index}">${tarotCardFace(card, true)}<span>${escapeHtml(card.name)} · ${card.reversed ? "逆位" : "正位"}</span></div>`).join("")}</div></div>`;
  }
  if (message.type === "shop-request") {
    return `<div class="bubble play-card shop-link-card"><div class="play-card-head"><span>FOR YOU · SHOP</span><i>${message.status === "resolved" ? "已选购" : "等待他挑选"}</i></div><strong>给你开放了一次选购权限</strong><small>去挑一件现在真正想要的东西。</small><div class="shop-link-line"><span>进入商城</span><b>›</b></div></div>`;
  }
  if (message.type === "shop-result") {
    if (payload.empty) return `<div class="bubble play-card shop-result-card"><div class="play-card-head"><span>SHOPPING NOTE</span></div><strong>他看了一会儿，最后还是决定先把积分留下。</strong><small>等攒得再多一点，也许会遇见更想要的东西。</small></div>`;
    return `<div class="bubble play-card shop-result-card"><div class="play-card-head"><span>HE CHOSE ONE</span><i>−${Math.max(0, Number(payload.price) || 0)} 积分</i></div><div class="shop-result-product">${safeImage(payload.cover) ? `<img src="${payload.cover}" alt="">` : `<i>♡</i>`}<span><strong>${escapeHtml(payload.name || "一件礼物")}</strong><small>${escapeHtml(payload.description || "这份约定已经替他收好。")}</small></span></div></div>`;
  }
  return "";
}
...
function renderMessages(scrollToEnd = true) {
  ...
  $("messages").innerHTML = earlierControl + displayedMessages.map((message, index) => {
    ...
    const tarotResult = message.type === "tarot-result";
    return `${divider}<article class="message-row ${mine ? "me" : "lover"} ${tarotResult ? "tarot-message-row" : ""}" data-message-row="${message.id}">${mine ? actions : `<div class="message-avatar avatar lover-mark">${avatarMarkup(state.loverAvatar, state.loverName)}</div>`}<div class="message-body">${content}${tarotResult ? actions : ""}</div>${mine ? `<div class="message-avatar avatar me-mark">${avatarMarkup(state.myAvatar, state.myName)}</div>` : tarotResult ? "" : actions}</article>`;
  }).join("");
...
function pendingMessages(conversation = currentConversation()) {
  const messages = conversation?.messages || [];
  if (conversation && Object.prototype.hasOwnProperty.call(conversation, "replyHandledUserMessageId")) {
    const handledIndex = conversation.replyHandledUserMessageId
      ? messages.findIndex((message) => message.id === conversation.replyHandledUserMessageId)
      : -1;
    return messages.slice(handledIndex + 1).filter((message) => message.from === "me");
  }
  let lastLover = -1;
...
function queueAutoReply() {
  ...
  autoReplyTimer = null;
  if (!state.autoReplyEnabled) { autoReplyQueued = false; autoReplyConversationId = ""; return; }
  autoReplyConversationId = currentConversation()?.id || "";
  autoReplyQueued = true;
  autoReplyTimer = setTimeout(() => {
    autoReplyTimer = null;
    if (typing) return;
    if (!autoReplyConversationId || currentConversation()?.id !== autoReplyConversationId) { autoReplyQueued = false; autoReplyConversationId = ""; return; }
    autoReplyQueued = false;
    autoReplyConversationId = "";
    requestReply();
  }, 650);
}
...
function tarotResultMessage(request) {
  ...
  conversation.messages.push(message);
  conversation.replyHandledUserMessageId = request.id;
...
function toggleMorePopover() {
  ...
  popover.querySelectorAll("[data-more-action]").forEach((button) => button.addEventListener("click", () => {
    ...
    else if (action === "shop") sendShopRequest();
...
async function sendImage(event) {
  ...
  try {
    ...
    else if (conversation.id === state.activeConversationId) { renderMessages(); queueAutoReply(); }
...
function buildReplyItems(combined, conversationMessages = currentMessages(), mode = state.mode) {
  ...
  const pool = [...cardPool.map((card) => ({ kind: "card", card, sourceCards: [card] })), ...(state.replyStickersDisabled ? [] : state.stickers.map((sticker) => ({ kind: "sticker", sticker })))];
  ...
  if (mode === "response" && matchedSections.size && cardPool.length) {
    ...
    if (matchedPool.length) { const card = matchedPool[Math.floor(Math.random() * matchedPool.length)]; replies.push({ kind: "card", card, sourceCards: [card] }); }
  }
  while (replies.length < count) {
    ...
    if (picked.card.combo && Math.random() < 0.5) {
      ...
      if (followPool.length) {
        ...
        replies.push({ kind: "text", content: `${first} ${second}`, sourceCards: [picked.card, follow] });
        continue;
      }
    }
    replies.push(picked);
...
function requestReply() {
  ...
  const pending = pendingMessages(replyConversation);
  const handledUserMessageId = pending.at(-1)?.id || "";
  ...
  const shopRequest = pendingShopRequests(replyConversation).at(-1);
  if (shopRequest) {
    ensureReplyAudio();
    typing = true; $("typing").hidden = false; $("presence").textContent = "正在挑选…"; updateReplyButton(); scrollMessagesToEnd();
    const minimum = Math.min(60, Math.max(1, Number(state.replyDelayMin || 1)));
    const maximum = Math.min(120, Math.max(minimum, Number(state.replyDelayMax || minimum)));
    const delay = Math.round((minimum + Math.random() * (maximum - minimum)) * 1000);
    setTimeout(async () => {
      const messageLength = replyConversation.messages.length;
      const ledgerLength = state.pointLedger.length;
      const purchaseLength = state.purchasedItems.length;
      const products = eligibleShopProducts();
      const selection = products.length ? Math.floor(Math.random() * (products.length + 1)) : products.length;
      const product = selection < products.length ? products[selection] : null;
      shopRequest.status = "resolved";
      let payload = { empty: true };
      if (product) {
        const snapshot = { id: product.id, name: product.name, description: product.description, price: product.price, cover: product.cover };
        state.purchasedItems.push({ id: uid(), productId: product.id, name: product.name, description: product.description, price: product.price, cover: product.cover, boughtAt: new Date().toISOString(), usedAt: "" });
        addPointLedgerEntry({ amount: -product.price, type: "purchase", reason: "商城购买", note: product.name, sourceMessageId: `shop:${shopRequest.id}`, product: snapshot });
        payload = { empty: false, ...snapshot };
      }
      replyConversation.messages.push({ id: uid(), from: "lover", type: "shop-result", content: product ? `[商城] ${product.name}` : "[商城] 暂时没有选择", payload, interactionId: shopRequest.id, createdAt: new Date().toISOString() });
      if (handledUserMessageId) replyConversation.replyHandledUserMessageId = handledUserMessageId;
      replyConversation.updatedAt = new Date().toISOString();
      const saved = await saveStateNow(false);
      if (!saved) {
        shopRequest.status = "pending"; replyConversation.messages.splice(messageLength); state.pointLedger.splice(ledgerLength); state.purchasedItems.splice(purchaseLength);
      } else playReplySound();
      typing = false; $("typing").hidden = true; $("presence").textContent = "讯号在线"; updateReplyButton(); renderMessages();
      if (saved && state.autoReplyEnabled && replyConversation.id === state.activeConversationId && pendingMessages(replyConversation).length) queueAutoReply();
    }, delay);
    return;
  }
  ...
  const finishReply = () => {
    ...
    saveState();
    renderMessages();
    if (state.autoReplyEnabled && replyConversation.id === state.activeConversationId && pendingMessages(replyConversation).length) queueAutoReply();
  };
  const deliverReplyItem = (index) => {
    try {
      ...
      if (item.kind === "choice-result") {
        ...
        const responseMessage = { id: uid(), from: "lover", type: "text", content: item.card?.content || item.content, quote, createdAt: new Date().toISOString() };
        const pointReward = (item.sourceCards || (item.card ? [item.card] : [])).reduce((total, card) => total + parsePointCard(card?.content), 0);
        if (pointReward > 0) {
          responseMessage.payload = { pointReward };
          if (addPointLedgerEntry({ amount: pointReward, type: "card_reward", reason: "积分字卡", note: responseMessage.content, sourceMessageId: responseMessage.id })) showToast(`他收到了 ${pointReward} 枚积分`);
        }
        replyConversation.messages.push(responseMessage);
      }
      replyConversation.updatedAt = new Date().toISOString();
      saveState();
...
function renderDrawer(tool) {
  const names = { conversations: "", cards: "字卡", stickers: "表情包", shop: "商城", memories: "纪念日", appearance: "设置", data: "数据" };
...
function renderConversationsDrawer() {
  const items = [
    ...
    ["shop", "商城", "商品、积分和他的资产", '<path d="M4 8h16l-1 12H5L4 8ZM8 8a4 4 0 0 1 8 0"/>'],
...
function createConversation() {
  const conversation = { id: uid(), title: "新对话", createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), messages: [], replyHandledUserMessageId: "" };
...
function renderShopDrawer() {
  const balance = currentPointBalance();
  const products = [...state.shopProducts].sort((a, b) => a.sort - b.sort);
  const remaining = state.purchasedItems.filter((item) => !item.usedAt).sort((a, b) => new Date(b.boughtAt) - new Date(a.boughtAt));
  const used = state.purchasedItems.filter((item) => item.usedAt).sort((a, b) => new Date(b.usedAt) - new Date(a.usedAt));
  const ledger = [...state.pointLedger].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  const dateText = (value) => new Date(value).toLocaleString("zh-CN", { month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit" });
  const assetItem = (item, isUsed = false) => `<article class="asset-item">${safeImage(item.cover) ? `<img src="${item.cover}" alt="">` : `<i>♡</i>`}<span><strong>${escapeHtml(item.name)}</strong><small>${isUsed ? `已使用 · ${dateText(item.usedAt)}` : `购入于 ${dateText(item.boughtAt)}`}</small></span>${isUsed ? "" : `<button data-use-purchase="${item.id}">使用</button>`}</article>`;
  const productEditor = (product) => `<details class="shop-product" data-product="${product.id}"><summary>${safeImage(product.cover) ? `<img src="${product.cover}" alt="">` : `<i>♡</i>`}<span><strong>${escapeHtml(product.name)}</strong><small>${product.price} 积分 · ${product.enabled ? "已上架" : "未上架"}</small></span><b>⌄</b></summary><div class="shop-product-fields"><label><span>商品名称</span><input data-product-field="name" value="${escapeHtml(product.name)}" maxlength="80"></label><label><span>说明或约定</span><textarea data-product-field="description" maxlength="300" placeholder="写下这张卡可以怎样使用">${escapeHtml(product.description)}</textarea></label><div class="shop-price-row"><label><span>价格</span><input data-product-field="price" type="number" min="1" max="999999" inputmode="numeric" value="${product.price}"></label><label><span>排序</span><input data-product-field="sort" type="number" value="${product.sort}"></label></div><div class="shop-product-cover"><label class="wallpaper-button">${safeImage(product.cover) ? "更换封面" : "添加封面"}<input type="file" accept="image/*" data-product-cover="${product.id}"></label>${safeImage(product.cover) ? `<button class="restore-button" data-remove-product-cover="${product.id}">移除封面</button>` : ""}</div><div class="shop-product-toggles"><label><input data-product-field="enabled" type="checkbox" ${product.enabled ? "checked" : ""}> 上架商品</label><label><input data-product-field="allowRepeat" type="checkbox" ${product.allowRepeat ? "checked" : ""}> 允许重复购买</label></div><div class="shop-product-actions"><button class="danger-button" data-delete-product="${product.id}">删除</button><button class="secondary-button" data-save-product="${product.id}">保存商品</button></div></div></details>`;
  $("drawerContent").innerHTML = `
    <section class="shop-hero"><span>HIS LITTLE TREASURY</span><small>他现在拥有</small><strong>${balance}<i>积分</i></strong><p>每一枚都来自被记住的认真，或一次毫无道理的偏爱。</p></section>
    <details class="shop-assets"><summary><span><strong>他的资产</strong><small>${remaining.length} 件待使用 · ${used.length} 件已兑现</small></span><b>⌄</b></summary><div class="shop-assets-body"><section class="shop-reward"><div class="section-title"><strong>奖励他一点</strong><span>会留在积分流水里</span></div><div class="shop-reward-row"><input id="manualRewardAmount" type="number" min="1" max="9999" inputmode="numeric" placeholder="积分"><select id="manualRewardReason"><option value="因为溺爱">因为溺爱</option><option value="确实值得奖励">确实值得奖励</option></select></div><input id="manualRewardNote" maxlength="120" placeholder="想记下原因的话，可以写在这里"><button class="secondary-button" id="grantManualReward">把积分送给他</button></section><section><div class="section-title"><strong>还没有使用</strong><span>${remaining.length} 件</span></div>${remaining.length ? `<div class="asset-list">${remaining.map((item) => assetItem(item)).join("")}</div>` : '<div class="shop-empty">还没有被他带回来的约定。</div>'}</section><details class="asset-history"><summary>使用过的物品 <b>${used.length}</b></summary>${used.length ? `<div class="asset-list">${used.map((item) => assetItem(item, true)).join("")}</div>` : '<div class="shop-empty">兑现过的约定会留在这里。</div>'}</details><details class="asset-history"><summary>积分流水 <b>${ledger.length}</b></summary>${ledger.length ? `<div class="point-ledger">${ledger.map((item) => `<article><span><strong>${item.amount > 0 ? "+" : ""}${item.amount}</strong><small>${escapeHtml(item.reason || ({ card_reward: "积分字卡", purchase: "商城购买" }[item.type] || "积分调整"))}${item.note ? ` · ${escapeHtml(item.note)}` : ""}</small></span><time>${dateText(item.createdAt)}</time></article>`).join("")}</div>` : '<div class="shop-empty">积分第一次抵达时，会从这里开始记录。</div>'}</details></div></details>
    <section class="drawer-section shop-catalog"><div class="section-title"><strong>选品</strong><span>${products.filter((product) => product.enabled).length} 件已上架</span></div><p class="drawer-intro">把想给他的约定做成商品。发送商城邀请后，他只会从当时买得起的商品中挑选一件。</p><button class="shop-add-product" id="addShopProduct">＋ 添加一件商品</button><div class="shop-product-list">${products.length ? products.map(productEditor).join("") : '<div class="shop-empty shop-empty-products">这里还没有商品。等你亲手放进第一份约定。</div>'}</div></section>`;
  $("grantManualReward").addEventListener("click", async () => {
    const amount = Math.min(9999, Math.max(1, Math.trunc(Number($("manualRewardAmount").value) || 0)));
    if (!Number($("manualRewardAmount").value)) { showToast("先写下想奖励他的积分"); return; }
    addPointLedgerEntry({ amount, type: "manual_reward", reason: $("manualRewardReason").value, note: $("manualRewardNote").value.trim() });
    if (await saveStateNow(false)) { showToast(`已经把 ${amount} 枚积分送给他`); renderShopDrawer(); }
  });
  document.querySelectorAll("[data-use-purchase]").forEach((button) => button.addEventListener("click", () => {
    const item = state.purchasedItems.find((entry) => entry.id === button.dataset.usePurchase);
    if (!item) return; item.usedAt = new Date().toISOString(); saveState(); showToast("这份约定已经兑现"); renderShopDrawer();
  }));
  $("addShopProduct").addEventListener("click", () => {
    state.shopProducts.push({ id: uid(), name: "未命名商品", description: "", price: 1, cover: "", enabled: false, allowRepeat: true, sort: state.shopProducts.length });
    saveState(); renderShopDrawer(); document.querySelector(".shop-product-list .shop-product:last-child")?.setAttribute("open", "");
  });
  document.querySelectorAll("[data-save-product]").forEach((button) => button.addEventListener("click", async () => {
    const product = state.shopProducts.find((item) => item.id === button.dataset.saveProduct); const root = button.closest("[data-product]");
    if (!product || !root) return;
    const read = (key) => root.querySelector(`[data-product-field="${key}"]`);
    const name = read("name").value.trim(); if (!name) { showToast("商品需要一个名字"); return; }
    Object.assign(product, { name, description: read("description").value.trim(), price: Math.max(1, Math.trunc(Number(read("price").value) || 1)), sort: Number(read("sort").value) || 0, enabled: read("enabled").checked, allowRepeat: read("allowRepeat").checked });
    if (await saveStateNow(false)) { showToast("商品已经替你收好"); renderShopDrawer(); }
  }));
  document.querySelectorAll("[data-product-cover]").forEach((input) => input.addEventListener("change", async (event) => {
    const product = state.shopProducts.find((item) => item.id === event.target.dataset.productCover); const file = event.target.files[0]; if (!product || !file) return;
    if (file.size > 8 * 1024 * 1024) { showToast("封面请控制在 8MB 以内"); return; }
    product.cover = await compressImage(file, 800, .84); if (await saveStateNow(false)) renderShopDrawer();
  }));
  document.querySelectorAll("[data-remove-product-cover]").forEach((button) => button.addEventListener("click", () => { const product = state.shopProducts.find((item) => item.id === button.dataset.removeProductCover); if (product) { product.cover = ""; saveState(); renderShopDrawer(); } }));
  document.querySelectorAll("[data-delete-product]").forEach((button) => button.addEventListener("click", () => {
    const product = state.shopProducts.find((item) => item.id === button.dataset.deleteProduct); if (!product || !confirm(`删除商品“${product.name}”吗？已经购买的记录不会消失。`)) return;
    state.shopProducts = state.shopProducts.filter((item) => item.id !== product.id); saveState(); renderShopDrawer();
  }));
}

function renderCardsDrawer() {
  const query = cardQuery.trim();
  const enabledCardCount = state.cards.filter((card) => state.sectionSettings[card.section]?.enabled !== false && card.enabled !== false).length;
  ...
  const groupedCards = visibleSections.map((name) => {
    ...
    const fixed = state.fixedSections.includes(name);
    ...
    return `<details class="card-group ${fixed ? "is-fixed" : ""} ${rules.enabled ? "" : "is-disabled"}" ${query ? "open" : ""}><summary><span><strong>${escapeHtml(name)}<i class="section-badges">${fixed ? '<em class="fixed-badge">固定</em>' : ""}${rules.enabled ? "" : '<em class="disabled-badge">未启用</em>'}${rules.combo ? '<em class="combo-badge">可组合</em>' : ""}</i></strong><small data-section-count="${escapeHtml(name)}">${cards.length} 张字卡</small></span><b>⌄</b></summary><div class="card-group-editor"><div class="section-rule-toggles">${ruleToggle("random", "随机")}${ruleToggle("response", "回应")}${ruleToggle("enabled", "启用")}${ruleToggle("combo", "组合")}</div><label class="section-trigger-setting"><span>回应关键词</span><input data-section-triggers="${escapeHtml(name)}" value="${escapeHtml((rules.triggers || []).join("，"))}" placeholder="例如：想你，晚安，难过" spellcheck="false" autocomplete="off"></label><p>${name === "积分" ? "使用“积分+数字”的格式，最多两位小数，例如：积分+1.5。回复真正发出后才会增加积分。" : "回应模式始终保留所有已开启分区；同时命中多个分区时，会合并成一个候选池并从中保证抽取一条。"}一行一张字卡，可直接在下方增删修改。</p><textarea class="section-card-editor" data-card-section="${escapeHtml(name)}" placeholder="${name === "积分" ? "积分+0.5\n积分+1.5\n积分+3" : "在这里输入字卡，一行一张"}" spellcheck="false" autocapitalize="off" autocomplete="off">${escapeHtml(contents)}</textarea><button class="group-save-button" data-save-section="${escapeHtml(name)}">保存这个分区</button></div></details>`;
  }).join("");
  $("drawerContent").innerHTML = `
    <section class="drawer-section"><div class="section-title"><strong>分区</strong><span>${state.sections.length} 个</span></div><div class="card-library-stats"><span><b data-total-card-count>${state.cards.length}</b><small>全部字卡</small></span><span><b data-enabled-card-count>${enabledCardCount}</b><small>当前启用</small></span></div><div class="inline-form"><input id="newSection" placeholder="新分区名称"><button class="secondary-button" id="addSection">添加</button></div><div class="section-tags">${state.sections.map((name) => `<span class="section-chip ${state.fixedSections.includes(name) ? "fixed" : ""}">${escapeHtml(name)}${!state.fixedSections.includes(name) && state.sections.length > 1 ? `<button data-delete-section="${escapeHtml(name)}">×</button>` : ""}</span>`).join("")}</div></section>
    <section class="drawer-section duplicate-check"><div class="section-title"><strong>全库检查</strong><span>不区分分组</span></div><div class="duplicate-keyword-search"><input id="cardKeywordLookup" value="${escapeHtml(cardKeywordQuery)}" placeholder="查找某个词是否存在" spellcheck="false"><button class="secondary-button" id="searchCardKeyword">查找</button></div><div id="keywordLookupResults"></div><button class="secondary-button" id="checkDuplicates">检查全部 ${state.cards.length} 张字卡是否重复</button><div id="duplicateResults"></div></section>
  ...
  const searchCardKeyword = () => {
    cardKeywordQuery = $("cardKeywordLookup").value.trim();
    const keyword = cardKeywordQuery;
    ...
    $("keywordLookupResults").innerHTML = matches.length
      ? `<div class="keyword-summary">找到 ${matches.length} 张包含“${escapeHtml(keyword)}”的字卡</div><div class="keyword-result-list">${matches.map((card) => `<article data-keyword-card="${card.id}"><header><span>${escapeHtml(card.section)}</span><small>${state.sectionSettings[card.section]?.enabled === false ? "未启用" : "已启用"}</small></header><textarea data-keyword-card-content spellcheck="false">${escapeHtml(card.content)}</textarea><div class="keyword-card-actions"><button data-save-keyword-card="${card.id}" aria-label="保存修改" title="保存修改"><svg viewBox="0 0 24 24"><path d="M5 12.5l4 4L19 7"/></svg></button><button data-delete-keyword-card="${card.id}" aria-label="删除字卡" title="删除字卡"><svg viewBox="0 0 24 24"><path d="M5 7h14M9 7V4h6v3M8 10v8M12 10v8M16 10v8M7 7l1 14h8l1-14"/></svg></button></div></article>`).join("")}</div>`
      : `<div class="keyword-empty">没有找到包含“${escapeHtml(keyword)}”的字卡。</div>`;
    document.querySelectorAll("[data-save-keyword-card]").forEach((button) => button.addEventListener("click", async () => {
      const card = state.cards.find((item) => item.id === button.dataset.saveKeywordCard);
      const textarea = button.closest("[data-keyword-card]")?.querySelector("[data-keyword-card-content]");
      const content = textarea?.value.replace(/\r?\n+/g, " ").trim() || "";
      if (!card || !content) { showToast("字卡内容不能为空"); return; }
      const previous = card.content;
      card.content = content;
      button.disabled = true;
      const saved = await saveStateNow(false);
      button.disabled = false;
      if (!saved) { card.content = previous; if (textarea) textarea.value = previous; return; }
      showToast("这张字卡已替换");
      searchCardKeyword();
    }));
    document.querySelectorAll("[data-delete-keyword-card]").forEach((button) => button.addEventListener("click", async () => {
      const card = state.cards.find((item) => item.id === button.dataset.deleteKeywordCard);
      if (!card) return;
      const previousCards = state.cards;
      state.cards = state.cards.filter((item) => item.id !== card.id);
      button.disabled = true;
      if (!await saveStateNow(false)) { state.cards = previousCards; button.disabled = false; return; }
      showToast("这张字卡已删除");
      renderCardsDrawer();
    }));
  };
  ...
  if (cardKeywordQuery) searchCardKeyword();
  ...
  document.querySelectorAll("[data-section-rule]").forEach((input) => input.addEventListener("change", () => {
    ...
    saveState();
    if (rule === "combo" || rule === "enabled") {
      const group = input.closest(".card-group");
      const badges = group?.querySelector(".section-badges");
      const rules = state.sectionSettings[section];
      group?.classList.toggle("is-disabled", !rules.enabled);
      if (badges) badges.innerHTML = `${state.fixedSections.includes(section) ? '<em class="fixed-badge">固定</em>' : ""}${rules.enabled ? "" : '<em class="disabled-badge">未启用</em>'}${rules.combo ? '<em class="combo-badge">可组合</em>' : ""}`;
      const enabledCountNode = document.querySelector("[data-enabled-card-count]");
      if (enabledCountNode) enabledCountNode.textContent = String(state.cards.filter((card) => state.sectionSettings[card.section]?.enabled !== false && card.enabled !== false).length);
    }
  }));
  ...
  document.querySelectorAll("[data-save-section]").forEach((button) => button.addEventListener("click", async () => {
    ...
    if (section === "积分" && lines.some((line) => !parsePointCard(line))) { showToast("积分字卡请使用“积分+数字”的格式，最多两位小数"); return; }
    ...
    if (!saved) { state.cards = previousCards; state.sectionSettings[section] = previousRules; }
      ...
      const totalCountNode = document.querySelector("[data-total-card-count]");
      const enabledCountNode = document.querySelector("[data-enabled-card-count]");
      if (totalCountNode) totalCountNode.textContent = String(state.cards.length);
      if (enabledCountNode) enabledCountNode.textContent = String(state.cards.filter((card) => state.sectionSettings[card.section]?.enabled !== false && card.enabled !== false).length);
      if ($("checkDuplicates")) $("checkDuplicates").textContent = `检查全部 ${state.cards.length} 张字卡是否重复`;
  ...
  document.querySelectorAll("[data-delete-section]").forEach((button) => button.addEventListener("click", () => {
    ...
    if (state.fixedSections.includes(name)) return;
...
function renderAppearanceDrawer() {
  $("drawerContent").innerHTML = `
    ...
    <section class="drawer-section choice-card-settings"><div class="section-title"><strong>功能卡片</strong><span>共用同一个底色</span></div><div class="choice-card-live-preview"><article><small>我问你 · 商城 · 塔罗邀请</small><strong>今天想去哪里？</strong><span>○ 海边</span><span>○ 山里</span></article><article><small>商城选购</small><strong>一份只属于你的约定</strong></article></div>${colorSettingHtml("统一卡片底色", "choiceCardColor")}<button class="restore-button color-reset" id="resetChoiceCardColor">恢复默认卡片底色</button></section>
