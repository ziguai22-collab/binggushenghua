...
function currentPointBalance() {
  return Math.max(0, state.pointLedger.reduce((total, item) => total + Number(item.amount || 0), 0));
}

function parsePointCard(content) {
  const matched = String(content || "").trim().match(/^积分\s*\+\s*(\d+)$/);
  if (!matched) return 0;
  return Math.min(9999, Math.max(1, Number(matched[1]) || 0));
}
...
function normalizeState(saved = {}) {
    ...
    migrated.pointLedger = Array.isArray(saved.pointLedger) ? saved.pointLedger.map((item) => ({
      id: item.id || uid(), amount: Math.trunc(Number(item.amount) || 0), type: String(item.type || "adjustment"), reason: String(item.reason || ""), note: String(item.note || ""),
...
function renderCardsDrawer() {
  ...
  document.querySelectorAll("[data-save-section]").forEach((button) => button.addEventListener("click", async () => {
    ...
    if (section === "积分" && lines.some((line) => !parsePointCard(line))) { showToast("积分字卡请使用“积分+数字”的格式"); return; }
