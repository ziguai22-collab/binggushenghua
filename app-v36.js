...
const rgbToHex = (red, green, blue) => `#${[red, green, blue].map((channel) => Math.min(255, Math.max(0, Number(channel) || 0)).toString(16).padStart(2, "0")).join("")}`;
...
async function persistState(quiet = true) {
  ...
  const snapshot = structuredClone({ ...state, version: 14, lastSavedAt: savedAt });
  const task = async () => {
    try {
...
function renderPlayMessage(message) {
  ...
  if (message.type === "choice-result") {
    ...
    return `<div class="bubble play-card choice-result-card"><div class="play-card-head"><span>你的回答 · ${choiceModeLabel(payload.mode)}</span></div><small>${escapeHtml(payload.question || "这一次的选择")}</small>${payload.mode === "ranking" ? `<ol class="choice-ranking">${selected.map((option, index) => `<li><b>${index + 1}</b><span>${escapeHtml(option)}</span></li>`).join("")}</ol>` : `<div class="choice-selected">${selected.map((option) => `<span>${escapeHtml(option)}</span>`).join("")}</div>`}</div>`;
  }
  if (message.type === "dice") {
    ...
    return `<div class="bubble play-throw dice-card"><div class="die-scene"><div class="die-result ${animate ? "rolling" : ""}" data-value="${value}" aria-label="骰子 ${value} 点">${diceFace(value)}</div></div><strong>掷出 ${value}</strong></div>`;
  }
  if (message.type === "rps") {
    ...
    const animate = Date.now() - new Date(message.createdAt).getTime() < 10000 && !animatedPlayMessages.has(message.id);
    return `<div class="bubble play-throw rps-card ${animate ? "revealing" : ""}"><div class="gesture-mark">${gestureIcon(gesture)}</div><strong>${label}</strong></div>`;
  }
  return "";
}
...
function renderMessages(scrollToEnd = true) {
  ...
  $("messages").innerHTML = earlierControl + displayedMessages.map((message, index) => {
    ...
    const expandAction = image ? `<button class="message-expand" data-expand-message="${message.id}" aria-label="放大查看" title="放大查看"><svg viewBox="0 0 24 24"><path d="M9 4H4v5M15 4h5v5M9 20H4v-5M15 20h5v-5"/></svg></button>` : "";
...
function renderMemoriesDrawer() {
  ...
  const setMemoryColor = (value) => {
    ...
    state.memoryTextColor = color; syncColorControl("memoryTextColor", color);
    document.querySelector(".memory-hero")?.style.setProperty("--memory-color", color); saveState(); return true;
  };
  document.querySelector('[data-color-picker="memoryTextColor"]').addEventListener("input", (event) => setMemoryColor(event.target.value));
  document.querySelector('[data-color-hex="memoryTextColor"]').addEventListener("change", (event) => { if (!setMemoryColor(event.target.value.trim())) { event.target.value = state.memoryTextColor; showToast("请输入六位十六进制色号，例如 #403a38"); } });
  document.querySelectorAll('[data-rgb-key="memoryTextColor"]').forEach((input) => input.addEventListener("input", (event) => {
    const control = event.target.closest("[data-color-setting]");
    const values = Object.fromEntries(["r", "g", "b"].map((channel) => [channel, Number(control.querySelector(`[data-rgb-channel="${channel}"]`).value)]));
    setMemoryColor(rgbToHex(values.r, values.g, values.b));
  }));
...
function colorSettingHtml(label, key) {
  ...
  const slider = (channel, name) => `<label class="rgb-row"><span>${name}</span><input type="range" min="0" max="255" step="1" value="${channels[channel]}" data-rgb-key="${key}" data-rgb-channel="${channel}" aria-label="${label}${name}通道"><b data-rgb-value="${key}-${channel}">${channels[channel]}</b></label>`;
  return `<div class="color-setting" data-color-setting="${key}"><div class="color-setting-head"><span>${label}</span><input type="color" value="${value}" data-color-picker="${key}" aria-label="${label}"><input value="${value}" maxlength="7" data-color-hex="${key}" aria-label="${label}色号"></div><div class="rgb-mixer">${slider("r", "R")}${slider("g", "G")}${slider("b", "B")}</div></div>`;
}
...
function syncColorControl(key, value) {
  ...
  Object.entries(channels).forEach(([channel, channelValue]) => {
    const range = control.querySelector(`[data-rgb-channel="${channel}"]`);
    const output = control.querySelector(`[data-rgb-value="${key}-${channel}"]`);
    if (range) range.value = channelValue;
    if (output) output.textContent = channelValue;
  });
}

function commitSettingColor(key, value) {
  const color = safeColor(value);
  if (!color) return false;
  state[key] = color;
...
function renderAppearanceDrawer() {
  $("drawerContent").innerHTML = `
    ...
    <section class="drawer-section choice-card-settings"><div class="section-title"><strong>“我问你”卡片</strong><span>底色即时预览</span></div><div class="choice-card-live-preview"><article><small>我问你 · 单选</small><strong>今天想去哪里？</strong><span>○ 海边</span><span>○ 山里</span></article><article><small>你的回答</small><strong>海边</strong></article></div>${colorSettingHtml("卡片底色", "choiceCardColor")}<button class="restore-button color-reset" id="resetChoiceCardColor">恢复默认卡片底色</button></section>
  ...
  document.querySelectorAll("[data-color-picker]").forEach((input) => input.addEventListener("input", (event) => {
    const key = event.target.dataset.colorPicker;
    commitSettingColor(key, event.target.value.toLowerCase());
  }));
  document.querySelectorAll("[data-color-hex]").forEach((input) => input.addEventListener("change", (event) => {
    ...
    commitSettingColor(key, color);
  }));
  document.querySelectorAll("[data-rgb-key]").forEach((input) => input.addEventListener("input", (event) => {
    const key = event.target.dataset.rgbKey;
    const control = event.target.closest("[data-color-setting]");
    const values = Object.fromEntries(["r", "g", "b"].map((channel) => [channel, Number(control.querySelector(`[data-rgb-channel="${channel}"]`).value)]));
    commitSettingColor(key, rgbToHex(values.r, values.g, values.b));
  }));
  ...
  $("resetChoiceCardColor").addEventListener("click", () => {
    state.choiceCardColor = ""; saveState(); applyAppearance(); renderAppearanceDrawer(); showToast("已恢复默认卡片底色");
  });
