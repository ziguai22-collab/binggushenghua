...
function renderPlayMessage(message) {
  ...
  if (message.type === "tarot-result") {
    ...
    return `<div class="bubble play-card tarot-result-card"><div class="play-card-head"><span>YOUR READING</span><i>${cards.length} 张</i></div>${payload.question ? `<small>${escapeHtml(payload.question)}</small>` : ""}<div class="tarot-result-preview">${preview.map((card) => tarotCardFace(card, true)).join("")}${cards.length > preview.length ? `<span class="tarot-more-count">＋${cards.length - preview.length}</span>` : ""}</div><p>${cards.map((card) => `${escapeHtml(card.name)}·${card.reversed ? "逆" : "正"}`).join("　")}</p></div>`;
  }
  return "";
}
...
function openTarotEditor() {
  ...
  $("tarotBlurRange").value = state.tarotBackgroundBlur;
  $("tarotOverlayRange").value = state.tarotBackgroundOverlay;
  $("tarotGlowRange").value = state.tarotGlow;
  $("tarotBlurValue").textContent = `${state.tarotBackgroundBlur}px`;
  $("tarotOverlayValue").textContent = `${state.tarotBackgroundOverlay}%`;
  $("tarotGlowValue").textContent = `${state.tarotGlow}%`;
...
function bindGlobalEvents() {
  ...
  [["tarotBlurRange", "tarotBackgroundBlur", "tarotBlurValue", "px"], ["tarotOverlayRange", "tarotBackgroundOverlay", "tarotOverlayValue", "%"], ["tarotGlowRange", "tarotGlow", "tarotGlowValue", "%"]].forEach(([id, key, output, unit]) => {
    $(id).addEventListener("input", (event) => { state[key] = Number(event.target.value); $(output).textContent = `${state[key]}${unit}`; applyTarotAppearance(); saveState(); });
  });
  $("tarotBackgroundFile").addEventListener("change", async (event) => {
    ...
    try {
      ...
      if (!await saveStateNow(false)) state.tarotBackgroundImage = previous;
      applyTarotAppearance();
    } catch { state.tarotBackgroundImage = previous; showToast("这张壁纸暂时无法读取"); }
    event.target.value = "";
  });
  $("removeTarotBackground").addEventListener("click", () => { state.tarotBackgroundImage = ""; applyTarotAppearance(); saveState(); });
