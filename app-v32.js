...
async function persistState(quiet = true) {
  ...
  const snapshot = structuredClone({ ...state, version: 12, lastSavedAt: savedAt });
  const task = async () => {
    try {
...
function buildReplyItems(combined, conversationMessages = currentMessages(), mode = state.mode) {
  ...
  if (mode === "response" && combined) {
    const matchedSections = new Set(state.sections.filter((section) => {
      const rules = state.sectionSettings[section];
      return rules?.enabled && rules?.response && (rules.triggers || []).some((word) => combined.includes(word));
    }));
    if (matchedSections.size) cardPool = eligibleCards.filter((card) => matchedSections.has(card.section));
  }
...
function requestReply() {
  ...
  const replyItems = buildReplyItems(combined, replyConversation.messages, replyMode);
  ensureReplyAudio();
  typing = true;
  $("typing").hidden = false;
  ...
  const deliverReplyItem = (index) => {
    try {
      ...
      if (item.kind === "sticker") replyConversation.messages.push({ id: uid(), from: "lover", type: "sticker", content: `[表情] ${item.sticker.name}`, dataUrl: item.sticker.dataUrl, quote, createdAt: new Date().toISOString() });
...
function renderCardsDrawer() {
  ...
  const groupedCards = visibleSections.map((name) => {
    ...
    return `<details class="card-group" ${query ? "open" : ""}><summary><span><strong>${escapeHtml(name)}</strong><small data-section-count="${escapeHtml(name)}">${cards.length} 张字卡</small></span><b>⌄</b></summary><div class="card-group-editor"><div class="section-rule-toggles">${ruleToggle("random", "随机")}${ruleToggle("response", "回应")}${ruleToggle("enabled", "启用")}${ruleToggle("combo", "组合")}</div><label class="section-trigger-setting"><span>回应关键词</span><input data-section-triggers="${escapeHtml(name)}" value="${escapeHtml((rules.triggers || []).join("，"))}" placeholder="例如：想你，晚安，难过" spellcheck="false" autocomplete="off"></label><p>关键词用逗号分隔；命中时优先进入这个分区，未命中时仍从全部回应分区随机。一行一张字卡，可直接在下方增删修改。</p><textarea class="section-card-editor" data-card-section="${escapeHtml(name)}" placeholder="在这里输入字卡，一行一张" spellcheck="false" autocapitalize="off" autocomplete="off">${escapeHtml(contents)}</textarea><button class="group-save-button" data-save-section="${escapeHtml(name)}">保存这个分区</button></div></details>`;
  }).join("");
...
function renderStickersDrawer() {
  $("drawerContent").innerHTML = `<section class="drawer-section sticker-upload-section"><div class="section-title"><strong>表情包</strong><span>支持 PNG、JPG、GIF、WebP</span></div><p class="drawer-intro">上传后会直接出现在聊天输入栏的笑脸按钮里，不需要再分类。</p><label class="upload-box">＋ 选择一张或多张图片<input id="stickerFiles" type="file" accept="image/*" multiple></label></section><section><div class="section-title"><strong>已保存</strong><span>${state.stickers.length} 张</span></div>${state.stickers.length ? `<div class="sticker-library-scroll"><div class="sticker-grid">${state.stickers.map((sticker) => `<article class="sticker-card" data-sticker="${sticker.id}"><img src="${sticker.dataUrl}" alt="${escapeHtml(sticker.name)}"><button data-delete-sticker="${sticker.id}" title="删除" aria-label="删除表情">×</button><input data-sticker-field="name" value="${escapeHtml(sticker.name)}" aria-label="表情名称"></article>`).join("")}</div></div>` : '<div class="empty-tool">还没有表情包。上传后可直接从聊天输入区发送。</div>'}</section>`;
