...
async function persistState(quiet = true) {
  ...
  const snapshot = structuredClone({ ...state, version: 13, lastSavedAt: savedAt });
  const task = async () => {
    try {
...
function diceFace(value) {
  return `<span class="die-face die-front" data-face="1"></span><span class="die-face die-right" data-face="2"></span><span class="die-face die-back" data-face="3"></span><span class="die-face die-left" data-face="4"></span><span class="die-face die-top" data-face="5"></span><span class="die-face die-bottom" data-face="6"></span>`;
}
...
function renderPlayMessage(message) {
  ...
  if (message.type === "choice-request") {
    ...
    return `<div class="bubble play-card choice-request-card"><div class="play-card-head"><span>CHOICE · ${choiceModeLabel(payload.mode)}</span><i>${message.status === "resolved" ? "已回答" : "等待回答"}</i></div><strong>${escapeHtml(payload.question || message.content)}</strong><ol>${options.map((option, index) => `<li><b>${payload.mode === "ranking" ? index + 1 : "○"}</b><span>${escapeHtml(option)}</span></li>`).join("")}</ol></div>`;
  }
  if (message.type === "choice-result") {
    const selected = Array.isArray(payload.selected) ? payload.selected : [];
    return `<div class="bubble play-card choice-result-card"><div class="play-card-head"><span>MY ANSWER · ${choiceModeLabel(payload.mode)}</span></div><small>${escapeHtml(payload.question || "这一次的选择")}</small>${payload.mode === "ranking" ? `<ol class="choice-ranking">${selected.map((option, index) => `<li><b>${index + 1}</b><span>${escapeHtml(option)}</span></li>`).join("")}</ol>` : `<div class="choice-selected">${selected.map((option) => `<span>${escapeHtml(option)}</span>`).join("")}</div>`}</div>`;
  }
  if (message.type === "dice") {
    ...
    return `<div class="bubble play-card dice-card"><div class="play-card-head"><span>DICE</span><i>${message.from === "me" ? state.myName : state.loverName}</i></div><div class="die-scene"><div class="die-cube value-${value} ${animate ? "rolling" : ""}" style="--die-final:var(--die-${value})">${diceFace(value)}</div></div><strong>掷出了 ${value}</strong></div>`;
  }
  if (message.type === "rps") {
    ...
    const animate = Date.now() - new Date(message.createdAt).getTime() < 10000 && !animatedPlayMessages.has(message.id);
    return `<div class="bubble play-card rps-card ${animate ? "revealing" : ""}"><div class="play-card-head"><span>ROCK · PAPER · SCISSORS</span></div><div class="gesture-mark">${gestureIcon(gesture)}</div><strong>${label}</strong></div>`;
  }
  return "";
}
...
function renderMessages(scrollToEnd = true) {
  ...
  $("messages").innerHTML = earlierControl + displayedMessages.map((message, index) => {
    ...
    const actions = `<div class="message-actions"><button class="message-quote" data-quote-message="${message.id}" aria-label="引用这条消息" title="引用这条消息"><svg viewBox="0 0 24 24"><path d="M10 8 5 12l5 4M6 12h7c3.4 0 5 1.8 5 5"/></svg></button><button class="message-delete" data-delete-message="${message.id}" aria-label="删除这条消息" title="删除这条消息"><svg viewBox="0 0 24 24"><path d="M7 7h10l-.7 12H7.7L7 7ZM9 7V4h6v3M5 7h14"/></svg></button></div>`;
  ...
  document.querySelectorAll(".media-bubble img").forEach((image) => image.addEventListener("click", (event) => {
    event.stopPropagation();
    openMediaViewer(image.src, image.alt);
  }));
...
function sendChoiceRequest() {
  ...
  const conversation = currentConversation();
  const message = { id: uid(), from: "me", type: "choice-request", content: `[指定选择] ${question}`, status: "pending", payload: { question, options, mode: selectedChoiceMode }, quote: takeManualReplyQuote(), createdAt: new Date().toISOString() };
...
function buildReplyItems(combined, conversationMessages = currentMessages(), mode = state.mode) {
  ...
  if (mode === "response" && matchedSections.size && cardPool.length) {
    matchedSections.forEach((section) => {
      const requiredPool = cardPool.filter((card) => card.section === section);
      if (requiredPool.length) replies.push({ kind: "card", card: requiredPool[Math.floor(Math.random() * requiredPool.length)] });
    });
  }
